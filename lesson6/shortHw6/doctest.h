...
#define DOCTEST_OUTPUT_DEBUG_STRING(text)
	...
	// =================================================================================================
		...
		std::string out_results_file_path = "results.json";

		struct CodallehReporter : public ConsoleReporter
		{
			std::ofstream m_fstream = std::ofstream(out_results_file_path);
			std::vector<CodallehTestResult> _testResults;
			std::stringstream _curr_test_err_msg;
			std::string _curr_test_name;

			CodallehReporter(const ContextOptions& co)
				: ConsoleReporter(co) {}

			CodallehReporter(const ContextOptions& co, std::ostream& ostr)
				: ConsoleReporter(co, ostr) {}

			~CodallehReporter() override {
				double num_tests = (double)_testResults.size();
				double gradePerTest = 100. / num_tests;
				double totalGrade = 0;
				for (auto& res : _testResults)
				{
					m_fstream << format_output(res, (int)ceil(gradePerTest));
					if (res.IsPass)
						totalGrade += gradePerTest;
				}
				if (totalGrade > 99.9)
					totalGrade = 100;

				m_fstream << "Grade :=>> " << (int)ceil(totalGrade) << std::endl;

				m_fstream.flush();
				m_fstream.close();
			}

			std::string format_output(CodallehTestResult result, int gradePerTest) {
				std::ostringstream output;
				int grade = result.IsPass ? gradePerTest : 0;
				output << "Comment :=>> " << result.Name << ": " << (result.IsPass ? "success" : "failure") << ". " << grade << " marks\n";

				if (!result.IsPass) { output << "<|-- \n" << result.Msg << "\n --|>\n"; }

				return output.str();
			}

			void test_case_start(const TestCaseData& in) override {
				_curr_test_err_msg.clear();
				_curr_test_err_msg.str("");
				_curr_test_name = in.m_name;
				ConsoleReporter::test_case_start(in);
			}

			void test_case_end(const CurrentTestCaseStats& st) override {
				CodallehTestResult result;
				result.IsPass = st.failure_flags == TestCaseFailureReason::Enum::None;
				result.Duration_ms = (unsigned int)st.seconds * 1000;
				result.Name = _curr_test_name;
				result.Msg = _curr_test_err_msg.str();
			...
			void log_assert(const AssertData& rb) override {
				...
				_curr_test_err_msg << failureString(rb.m_at) << ": " << rb.m_file << ":" << rb.m_line << " " << rb.m_expr << "\n";
			}

			void test_case_exception(const TestCaseException& e) override {
				ConsoleReporter::test_case_exception(e);
				_curr_test_err_msg 
		...
		DOCTEST_REGISTER_REPORTER("codalleh", 0, CodallehReporter);
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                                        \
	...
	int Context::run() {
		...
		if (p->filters[8].empty())
			p->filters[8].push_back("codalleh");
